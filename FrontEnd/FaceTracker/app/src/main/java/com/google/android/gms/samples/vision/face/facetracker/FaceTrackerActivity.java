...
public final class FaceTrackerActivity extends AppCompatActivity {
    ...
    public LineData addData(int[] x, int[] y){
        ...
        Log.d("DEBUG", Integer.toString(x.length));
        Log.d("DEBUG", Integer.toString(y.length));

        for(int i = 0; i < x.length; i ++){
            entries.add(new Entry(x[i], y[i]));
        }
        Log.d("DEBUG", "dataset" + Integer.toString(entries.size()));
    ...
    private class GraphicFaceTracker extends Tracker<Face> {
        ...
        GraphicFaceTracker(GraphicOverlay overlay) {
            mOverlay = overlay;
            mFaceGraphic = new FaceGraphic(overlay, 0, score);
        }
